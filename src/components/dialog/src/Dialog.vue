<template>
    <hamster-modal class="minimized" ref="dialog">
        ...
        <div v-if="form" class="modal-body modal-scroll">
            <template v-for="el in form">
                ...
                <div v-if="el.type === 'numeric'" style="margin-bottom: 10px">
                    <label v-html="el.label"></label>
                    <hamster-numeric v-model="el.model" :min="el.min" :max="el.max" :step="el.step"></hamster-numeric>
                </div>

                <div v-if="el.type === 'chips'" style="margin-bottom: 10px">
                    <label v-html="el.label"></label>
                    <hamster-chips v-model="el.model"></hamster-chips>
                </div>

                <label v-if="el.type === 'radio'" v-for="radio in el.items" class="item">
                    <div class="item-primary">
                        <hamster-radio v-model="el.model" :val="radio.value"></hamster-radio>
                    </div>
                ...
                <label v-if="el.type === 'checkbox'" v-for="checkbox in el.items" class="item">
                    <div class="item-primary">
                      <hamster-checkbox v-model="checkbox.model"></hamster-checkbox>
                    </div>
                ...
                <label v-if="el.type === 'toggle'" v-for="toggle in el.items" class="item">
                    ...
                    <div class="item-secondary">
                      <hamster-toggle v-model="toggle.model"></hamster-toggle>
                    </div>
                </label>
                ...
                <div v-if="el.type === 'rating'" style="margin-bottom: 10px">
                  <label v-html="el.label"></label>
                  <hamster-rating v-model="el.model" :max="el.max" :icon="el.icon" :style="{fontSize: el.size || '2rem'}"></hamster-rating>
                </div>
            </template>
        </div>
